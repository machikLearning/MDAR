...
<head>
	...
	<script type="text/javascript">
		...
		function appendParent(ATCCode, ATCName){
			var html = "<tr>";
			html += "<td name='tolerableCode'>" + ATCCode + "</td>";
			html += "<td name = 'tolerableName'>" + ATCName + "</td>";
			html += "<td><input type=button value='삭제' class='btn_save' onClick='removeRow(this)'</td>";
			html += "</tr>";
			$(opener.document).find("#tolerableTableBody").append(html);
			close();
		
		}
		
		function selectTolerable(ATCCode, ATCName) {
...
<body style=" min-width: 1024px;">
	<div id="searchtab">
	<div class="searchtab_ti">복용 ATC 검색</div>
		<form id="searchTolerable" name="searchTolerable" class="search_info" action="<c:url value="/prescription/searchTolerableResult"/>" method="POST">
			<li>
		...
		<table class="tbl_basic center" id="medicineTable" style="table-layout:fixed;">
	          <caption>
	              <strong>복용 ATC 검색 결과</strong>
	              <details>
	                  <summary>복용 ATC 검색 결과</summary>
	              </details>
	          </caption>
	             <thead>
	                 <tr>
	             ...
	             <c:choose>
             		<c:when test="${fn:length(atcList) > 0 }">
						...
						<tr>
							...
							<td><input type="button" value="선택" class="btn_save" onclick="selectTolerable('<c:out value="${atc.code }"/>','<c:out value="${atc.levelName }"/>')"/></td>
						</tr>
						</c:forEach>
             		</c:when>
             		<c:otherwise>
		...
		<ul class="paginate">
			<c:if test="${searchpaging.nowPageGroup > 1 }">
				<li class="dir prev">
					<a href="<c:url value="/prescription/searchTolerableResult" >
			...
			<c:if test="${searchpaging.nowPageGroup < searchpaging.pageGroupCount }" >
     			<li class="dir next"><a href="<c:url value="/prescription/searchTolerableResult">
