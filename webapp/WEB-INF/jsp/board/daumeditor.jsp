...
<head>
    ...
    <link rel="stylesheet" href="<c:url value="/js/DaumEditor-master/daumeditor/css/editor.css"/>" type="text/css" charset="utf-8"/>
    <script src="<c:url value="/js/DaumEditor-master/daumeditor/js/editor_loader.js?enviroment=production"/>" type="text/javascript" charset="utf-8"></script>
	...
	<script type="text/javascript">
		...
		function modifyMode(){
			var form = $("#tx_editor_form");
			form.append($("#modifyBoardID").html());
			form.attr("action",'<c:url value="/board/insertBody"/>');
			$("#name").val($("#modifyTitle").text());
			modifyEditor();
			$("#submitFormButton").text("수정하기");
		}
		
		function modifyEditor(){
			Editor.modify({
				"content": $("#modifyContent").text()
				});
		}
...
<div class="body">
	...
	<div id="contentsWrap">
			...
			<!-- container :s -->
			<div id="container">
				<form name="tx_editor_form" id="tx_editor_form" action="<c:url value="/board/insertBody"/>" method="post" accept-charset="utf-8">
					<div> 제목 : <input name="name" id="name" type="text"></div>
					<input name="boardID" id="boardID"type="text" style="display:none">
					<input name="parentID" value="<c:out value="${parentID}"/>" type="hidden"/>
					<input name="depth" value="<c:out value="${depth}"/>" type="hidden"/>
					<br>
		<!-- 에디터 컨테이너 시작 -->
	...
	<div><button id="submitFormButton" onclick='saveContent()' class="mainbtn">SAMPLE - submit contents</button></div>
	
	<div id="modifyData" style="display:none">
		<input id="modifyBoardID" value="<c:out value='${boardDataType.boardID}'/>" type="text"/>
		<div id="modifyTitle"><c:out value='${boardDataType.boardTitle}'/></div>
		<div id="modifyContent"><c:out value='${boardDataType.boardBody}'/></div>
		<c:forEach var="file" items="${boardDataType.fileList}">
			<div class="fileDataType">
				<div class="fileNickName"><c:out value='${file.nickName}'/></div>
				<div class="fileSaveName"><c:out value='${file.saveName}'/></div>
			</div>
		</c:forEach>
	</div>
		...
		sidebar: {
			...
			attacher:{
				image:{ 
					...
					popPageUrl: "<c:url value='/board/boardImagePopup'/>" //팝업창 주소 
				},
				file:{
					features:{left:250,top:65,width:400,height:190,scrollbars:0}, //팝업창 사이즈 
					popPageUrl: "<c:url value='/board/boardFilePopup'/>" //팝업창 주소
				}
			},
